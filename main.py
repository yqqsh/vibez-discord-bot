...
async def get_help_embed():
    em = discord.Embed(title="Help Section", description="", color=discord.Color.purple())
    em.description += f"**{bot.command_prefix}join** : Joins Bot to the Current Voice channel you are in \n"
    em.description += f"**{bot.command_prefix}search <name/link>** : Searches for the song you request \n"
    em.description += f"**{bot.command_prefix}play <name>** : Plays song that is picked \n"
    em.description += f"**{bot.command_prefix}pause** : Pauses current song that is playing\n"
    em.description += f"**{bot.command_prefix}resume** : Resumes current song that is paused \n"
    em.description += f"**{bot.command_prefix}skip**: Creates 10 second poll for skipping the song that is being played, if 80%+ of the people in the vc vote for a skip, it will skip the song \n"
...
async def on_ready():
    ...
    await bot.change_presence(status = discord.Status.online, activity = discord.Game('Fire Music 🔥 | @Vibez'))
